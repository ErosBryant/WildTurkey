...
STOP_AFTER_MODEL_STATS="${STOP_AFTER_MODEL_STATS:-1}"
...
DEFAULT_ENGINES=$'bourbon:--mod=7 --always_learn=1\nwildturkey:--mod=10'
mapfile -t engines <<< "${ENGINES:-${DEFAULT_ENGINES}}"
...
benchmarks_for_mode() {
  if [[ "${ACTIVE_DATA_MODE}" == "real" ]]; then
    printf "fillrandom_r,waitbg,stats"
  else
    printf "fillrandom,waitbg,stats"
  fi
}

log_has_model_stats() {
  local log_path="$1"
  [[ -f "${log_path}" ]] && grep -q "^ModelTrainingStats " "${log_path}"
}
...
run_db_bench_case() {
  ...
  benchmark_arg="$(benchmarks_for_mode)"
  ...
  while kill -0 "${pid}" 2>/dev/null; do
    if log_has_model_stats "${log_path}"; then
      if [[ "${STOP_AFTER_MODEL_STATS}" == "1" ]]; then
      ...
      if [[ "${ready_since}" -eq 0 ]]; then
        ...
        echo "warning: db_bench produced model stats but did not exit after ${EXIT_GRACE_SECONDS}s; terminating pid ${pid}: ${log_path}" >&2
        kill "${pid}" 2>/dev/null || true
        sleep 2
        kill -9 "${pid}" 2>/dev/null || true
        wait "${pid}" 2>/dev/null
...
for engine_entry in "${engines[@]}"; do
  ...
  for dataset in "${datasets[@]}"; do
    for run in $(seq 1 "${RUNS}"); do
      ...
      echo "=== model-training ${engine} dataset=${dataset} run=${run} ==="
      set +e
      ...
      if [[ "${status}" != "0" ]]; then
        if log_has_model_stats "${log_path}"; then
          echo "warning: ${engine} produced model stats but exited with status ${status}: ${log_path}" >&2
          status="ok-with-exit-${status}"
        fi
      fi

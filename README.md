...
#### Built-in `db_bench` Random-Key Workloads

Use these commands for the standard LevelDB-style benchmark workloads generated by `db_bench` itself. They do not require external SOSD dataset files.
...
Use this path when you want benchmarks over real key distributions instead of `db_bench` generated random keys. Run `bash datasets/download.sh` and keep the generated binary key files under `datasets/data/`, or pass `--path_real_data=/path/to/data`. Use `fillrandom_r,readrandom_r` with explicit `--dataset`, `--dataset_size`, and `--num`, for example:

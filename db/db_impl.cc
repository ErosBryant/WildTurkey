  ...
  for (size_t i = 0; i < filenames.size(); i++) {
    if (ParseFileName(filenames[i], &number, &type)) {
      ...
      if (!keep) {
        ...
        auto model = adgMod::file_data->PeekModel(number);
        if (model && model->Learned()) {
          adgMod::file_data->ReleaseModel(number);
        }
...
void DBImpl::BackgroundCompaction() {
  ...
  if (c == nullptr) {
    ...
    if (status.ok()) {
      ...
      stats.max_bytes_read = f->file_size;
      stats.max_bytes_written = f->file_size;
      stats_[c->level() + 1].Add(stats);
    }
  ...
  if (c != nullptr && adgMod::MOD == 9 && !adgMod::fresh_write) {
      ...
      int level = c->level();
      adgMod::LearnedIndexData::LevelLearn(new adgMod::VersionAndSelf{current, version_count, current->learned_index_data_[level], level}, true);
      adgMod::LearnedIndexData::LevelLearn(new adgMod::VersionAndSelf{current, version_count, current->learned_index_data_[level+1], level+1}, true);
  }
      ...
      if (adgMod::MOD==10){
        // printf("sst_size = %d\n", adgMod::sst_size);
          const int output_level =
              std::min(compact->compaction->level() + 1,
                       leveldb::config::kNumLevels - 1);
          if (compact->builder->FileSize() >=
              compact->compaction->MaxOutputFileSizeineachlevel(output_level)) {
          status = FinishCompactionOutputFile(compact, input);
          if (!status.ok()) {
            break;
          }
        }
      }else if(adgMod::sst_size==4){
          const int output_level =
              std::min(compact->compaction->level() + 1,
                       leveldb::config::kNumLevels - 1);
          if (compact->builder->FileSize() >=
              compact->compaction->MaxOutputFileSizeLAC(output_level)) {
          status = FinishCompactionOutputFile(compact, input);
          if (!status.ok()) {
            break;
          }
...
if (adgMod::MOD == 10) {
  if (stats.bytes_written > 0) {
    // Level-0 compactions can report zero read_size; fall back to WA=1.0 to
    // keep the adaptive policy informed without dividing by zero.
    const double wa =
        read_size > 0
            ? static_cast<double>(stats.bytes_written) /
                  static_cast<double>(read_size)
            : 1.0;
    const uint64_t avg_output_size =
        compact->outputs.empty()
            ? 0
            : static_cast<uint64_t>(stats.bytes_written /
                                    static_cast<int64_t>(compact->outputs.size()));
    const int compaction_level = compact->compaction->level();
    const double ops_total =
        static_cast<double>(adgMod::num_read.load(std::memory_order_relaxed) +
                            adgMod::num_write.load(std::memory_order_relaxed));
    adgMod::GetAdaptiveLAC().Observe(
        compaction_level, avg_output_size, wa,
        static_cast<uint64_t>(stats.micros), ops_total);
  }
}
...
Status DBImpl::Get(const ReadOptions& options, const Slice& key,
  ...
  adgMod::Stats* instance = adgMod::Stats::GetInstance();
  adgMod::num_read.fetch_add(1, std::memory_order_relaxed);
...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* updates) {
  ...
  if (status.ok() && updates != nullptr) {  // nullptr batch is for compactions
    ...
    const int batch_count = WriteBatchInternal::Count(updates);
    WriteBatchInternal::SetSequence(updates, last_sequence + 1);
    last_sequence += batch_count;
    adgMod::num_write.fetch_add(batch_count, std::memory_order_relaxed);
...
bool DBImpl::GetProperty(const Slice& property, std::string* value) {
  ...
  if (in.starts_with("num-files-at-level")) {
    ...
    char buf[256];
    int waf =  0;
    double bytes_written_total =  0;
    double bytes_read_total = 0;
    double NumLevelBytes_total = 0;
    ...
    for (int level = 0; level < config::kNumLevels; level++) {
      ...
      if (stats_[level].micros > 0 || files > 0) {
                const double wa = stats_[level].bytes_read > 0
                        ? static_cast<double>(stats_[level].bytes_written) /
                          static_cast<double>(stats_[level].bytes_read)
                        : 1.0;
                const double wa_display = wa + 1.0;
                snprintf(buf, sizeof(buf), "%3d  %8d %9.0f %8.0f %9ld    %8.0f  %9.0f   %8.0f  %8.0f  %6ld  %4.2f\n", 
                 ...
                 bytes_read_total+= stats_[level].bytes_read;
                 NumLevelBytes_total+= versions_->NumLevelBytes(level) /1024/1024 ;
    ...
    const double total_wa = bytes_read_total > 0

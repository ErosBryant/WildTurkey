...
void Get99PercentileLatency(ThreadState* /*thread*/) {
  auto print_percentile = [](std::vector<long long>& samples,
                             const char* label) {
    if (samples.empty()) {
      printf("%s 99 percentile latency: unavailable (no samples)\n", label);
      return;
    }
    ...
    printf("%s 99 percentile latency: %lld ns\n", label,
           static_cast<long long>(samples[index]));
  };

  print_percentile(latencies, "Read");
  print_percentile(latencies_w, "Write");
}

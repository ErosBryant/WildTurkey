...
uint64_t Compaction::MaxOutputFileSizeLAC(int level) {
...
uint64_t Compaction::MaxOutputFileSizeineachlevel(int level) {
    auto& lac = adgMod::GetAdaptiveLAC();
    const uint64_t fallback = lac.DefaultSizeForLevel(level);
    return lac.GetTargetBytes(level, fallback);
}
    ...
    void Version::FileLearn() {
        for (int i = 0; i < config::kNumLevels; ++i) {
            for (FileMetaData *file_meta : files_[i]) {
                adgMod::LearnedIndexData::FileLearn(new adgMod::MetaAndSelf{this, adgMod::db->version_count, file_meta,
                                                                            adgMod::file_data->GetModel(
                                                                                    file_meta->number), i});
            }
        }
    }
